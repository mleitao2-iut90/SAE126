...
public abstract class Controller {
    ...
    public void setCombinaison(){;
        ...
        System.out.println("Donner une combinaison de 4 quatres couleurs parmi celle ci : \n  Rouge = R\n  Bleu = B\n  Jaune = J" +
                "\n  Vert = V\n  Blanc = W\n  Noir = N\n  Cyan = C\n  Violet = P");
        do{
            this.combinaison = input.next();
            if(goodValEnter(combinaison, true)){
                valide = true;
            }
        }while(!valide);
        System.out.println("Combinaison Bonne");
    }
        ...
        if(line.length() == 4){
            for(int i = 0; i < line.length(); i++){
                if(line.charAt(i) == 'R' || line.charAt(i) == 'B' || line.charAt(i) == 'J' || line.charAt(i) == 'V' || line.charAt(i) == 'W' || line.charAt(i) == 'N' || line.charAt(i) == 'C' || line.charAt(i) == 'P'){
                    ...
                    System.out.println("Combinaison non valide, recommencer");
                }
            }
        }
        else if(mode){
